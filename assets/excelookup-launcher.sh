#!/bin/sh
# ExcelLookup 启动器(Debian 包安装为 /usr/bin/excelookup,真正的二进制在
# /usr/lib/excelookup/excelookup)。
#
...
#
# 脚本负责日志轮转,并把 EXCELOOKUP_LAUNCHER 传给程序;程序据此改为追加写入,
# 不会把刚写进去的链接器报错截断。直接运行裸二进制时,轮转由程序自己做。

set -u

program_name=excelookup

# 允许用环境变量直接指定二进制,便于在未安装的情况下验证启动器行为。
real_binary=${EXCELOOKUP_BIN:-}
...
if [ -n "${XDG_STATE_HOME:-}" ]; then
    ...
    log_file=/tmp/excelookup-startup.log
fi
...
if [ -n "$log_file" ]; then
    ...
    export EXCELOOKUP_STDERR_IN_LOG=1
