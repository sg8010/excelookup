...
//! 因此把 `usr/bin/excelookup` 装成启动器脚本，由脚本把这些报错重定向进同一个
//! 日志文件（见 `assets/excelookup-launcher.sh`）。
...
const LAUNCHER_ENV: &str = "EXCELOOKUP_LAUNCHER";
...
/// 日志候选路径。启动器脚本 `assets/excelookup-launcher.sh` 使用同样的顺序,
...
const STDERR_IN_LOG_ENV: &str = "EXCELOOKUP_STDERR_IN_LOG";
...
mod tests {
    ...
    fn previous_log_path_keeps_directory() {
        assert_eq!(
            previous_log_path(Path::new("/home/u/.cache/excelookup/startup.log")),
            PathBuf::from("/home/u/.cache/excelookup/startup.log.1")
        );
    }
    ...
    fn temp_dir(name: &str) -> PathBuf {
        let dir =
            std::env::temp_dir().join(format!("excelookup-test-{}-{name}", std::process::id()));
        let _ = fs::remove_dir_all(&dir);
