...
//! 纯逻辑在 excelookup_lib::filebrowser(有单测)。
...
use excelookup_lib::filebrowser::{self, Entry, FilterOpts};
...
/// 读取用过滤器:Excel 全家桶 + 所有文件
pub fn workbook_filters() -> Vec<Filter> {
    vec![
        Filter::new(
            "Excel 工作簿 (*.xlsx, *.xls, *.xlsb, *.ods)",
...
/// 导出用过滤器(只写 xlsx)
pub fn xlsx_filters() -> Vec<Filter> {
    vec![Filter::new("Excel 工作簿 (*.xlsx)", &["xlsx"])]
}
...
impl FileDialog {
    ...
    pub fn ui(&mut self, ctx: &egui::Context) -> DialogAction {
        ...
        let modal = egui::Modal::new(egui::Id::new("excelookup_file_dialog"))
    ...
    fn body(&mut self, ui: &mut egui::Ui) -> DialogAction {
        ...
        ui.horizontal(|ui| {
            ...
            if self.filters.len() > 1 {
                ...
                egui::ComboBox::from_id_salt("excelookup_file_dialog_filter")
                    .width(300.0)
        ...
        if visible.is_empty() {
            ...
            let row_h = 24.0;
            TableBuilder::new(ui)
                .id_salt("excelookup_file_dialog_list")
                .striped(true)
                .vscroll(true)
...
mod tests {
    ...
    fn renders_every_mode_without_panic() {
        let mut open = FileDialog::open("选择工作簿", "数据源 A", None, workbook_filters());
        render(&mut open, 3);
        ...
        save.overwrite = Some(save.dir().join("连接结果.xlsx"));
        render(&mut save, 3);

        // 空列表(筛选词匹配不到任何名字)
        let mut filtered = FileDialog::open("选择工作簿", "数据源 B", None, workbook_filters());
        ...
        let mut fallback = FileDialog::open(
            "选择工作簿",
            "数据源 B",
            Some(PathBuf::from("/excelookup-不存在的目录")),
            workbook_filters(),
        );
