#!/usr/bin/env bash
# 交叉编译 Windows 7+ x64/x86 版 ExcelLookup
...
for TARGET in "${TARGETS[@]}"; do
    ...
    EXE="target/$TARGET/release/excelookup.exe"
    scripts/check-win7-imports.sh "$EXE" "$TARGET"

    mkdir -p "$DIST_DIR"
    OUT="$DIST_DIR/excelookup-win-$ARCH_SUFFIX.exe"
    cp -f "$EXE" "$OUT"
...
for TARGET in "${TARGETS[@]}"; do
    ...
    echo "   发布产物: $DIST_DIR/excelookup-win-$ARCH_SUFFIX.exe"
done
