...
- **键匹配智能**
  ...
  - 括号归一化:中文 `（）` 与英文 `()` 互认(可关闭)
  - 忽略案号分支后缀:匹配时忽略末尾的「之一」「之十二」等中文数字分支后缀,结果仍保留原值(默认关闭)
- **结果可控**:A、B 两侧都可通过标签勾选带出哪些列(A 默认全选,也可取消),A 匹配列默认带出、B 匹配列默认不带出，两侧至少保留一列匹配列在结果中;重复键自动展开多行
...
4. 勾选 A、B 中要**带出的列**(A 默认全选;B 匹配列默认不带出，两侧至少保留一列匹配列);按需开关「键宽松匹配」、
   「括号归一化」与「忽略案号分支后缀」
5. 点 **执行连接**,下方预览结果与统计
